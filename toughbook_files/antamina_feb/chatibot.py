...
class Camera1(QThread):
    change_pixmap1_signal = pyqtSignal(np.ndarray)
    ...
    def run(self):
        #Generate folder
        wd=os.getcwd()
        print("La direccion para camara 1 es: ",wd)
        numDir = 1
        countList = 0
        ...
        cap = cv2.VideoCapture("http://192.168.1.166:8085/?action=stream")
        while self._run_flag:
            ret, cv_img = cap.read()
            if ret:
                cv2.imwrite(nombreDirVid1 + str(imagen) + ".jpg",cv_img)
                self.change_pixmap1_signal.emit(cv_img)
                imagen=imagen+1
...
class Camera2(QThread):
    change_pixmap2_signal = pyqtSignal(np.ndarray)
    ...
    def run(self):
        wd=os.getcwd()
        print("La direccion para camara 2 es: ",wd)
        numDir = 2
        countList = 0
        contenido = os.listdir(wd)
        imagen=0
        ...
        # capture from web cam
        cap = cv2.VideoCapture("http://192.168.1.166:8080/?action=stream")
        while self._run_flag:
            ret, cv_img = cap.read()
            if ret:
                cv2.imwrite(nombreDirVid2 + str(imagen) + ".jpg",cv_img)
                self.change_pixmap2_signal.emit(cv_img)
                imagen=imagen+1
        # shut down capture system
        cap.release()
...
"}")
    ...
    def StartEndConexion(self):
        ...
        if CONN == 0:
            ...
            self.thread2 = Camera1()
            self.thread2.change_pixmap1_signal.connect(self.update1_image)
            self.thread3 = Camera2()
            self.thread3.change_pixmap2_signal.connect(self.update2_image)
            self.thread1.start()
            self.thread2.start()
            self.thread3.start()
            self.thread1.start()
    ...
    @pyqtSlot(np.ndarray)
    def update1_image(self, cv_img):
        """Updates the image_label with a new opencv image"""
        qt_img = self.convert1_cv_qt(cv_img)
        self.label_camera1.setPixmap(qt_img)
    
    def convert1_cv_qt(self, cv_img):
    ...
    def update2_image(self, cv_img):
        """Updates the image_label with a new opencv image"""
        qt_img = self.convert2_cv_qt(cv_img)
        self.label_camera2.setPixmap(qt_img)
    
    def convert2_cv_qt(self, cv_img):
        """Convert from an opencv image to QPixmap"""
        rgb_image = cv2.cvtColor(cv_img, cv2.COLOR_BGR2RGB)
        h, w, ch = rgb_image.shape
        bytes_per_line = ch * w
    ...
    def ActualizarLabel(self,textAction):
        ...
        if(textAction[5] == 'j'):
            ...
            self.ExpCom.appendPlainText("La nucleo esta enviando datos erroneos, revisar la roboclaw")
